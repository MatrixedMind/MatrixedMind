...
VALID = {"environment": "development", "project_id": "matrixed-mind-dev", "start_time": "2026-07-31T12:00:00Z", "end_time": "2026-07-31T12:30:00Z", "objective": "Check recent Cloud Run errors", "limit": 10}
...
class ObserverMcpTests(unittest.TestCase):
    def test_tool_list_is_exact_and_has_required_boundary_inputs(self) -> None:
        ...
        self.assertEqual(tools[4]["inputSchema"]["required"], [*observer.COMMON_REQUIRED, "alert_policy_id"])
        self.assertEqual(tools[7]["inputSchema"]["required"], [*observer.COMMON_REQUIRED, "service_name"])
        self.assertEqual(tools[8]["inputSchema"]["required"], [*observer.COMMON_REQUIRED, "location"])
    ...
    def test_rejects_unknown_mutation_and_bad_bounds(self) -> None:
        ...
        for bad in ({"project_id": "matrixedmind-prod"}, {"end_time": "2026-07-31T19:00:01Z"}, {"limit": 101}):
    ...
    def test_rejects_missing_tool_specific_or_arbitrary_arguments(self) -> None:
        for name, argument in (("list_timeseries", "filter"), ("query_range", "query"),
                               ("get_alert_policy", "alert_policy_id"), ("get_service", "service_name"),
    ...
    @patch.dict(os.environ, {"MATRIXEDMIND_GCP_OPERATOR_EMAIL": "operator@example.test"}, clear=True)
    @patch.object(observer, "command_token", side_effect=["source-token-should-never-leak", "impersonated-token-should-never-leak"])
    @patch.object(observer, "fetch_json")
    def test_log_result_is_sanitized_and_tokens_do_not_cross_boundary(self, fetch_json: object, _: object) -> None:
        fetch_json.side_effect = [
            {"email": "operator@example.test"},
            {"entries": [{"timestamp": "2026-07-31T12:00:01Z", "severity": "ERROR", "logName": "projects/matrixed-mind-dev/logs/run.googleapis.com%2Fstderr", "resource": {"type": "cloud_run_revision"}, "textPayload": "Bearer source-token-should-never-leak", "jsonPayload": {"secret": "no"}}]},
        ]
    ...
    @patch.object(observer, "fetch_json", side_effect=[{"email": "operator@example.test"}, {"entries": []}])
    def test_log_entries_inject_exact_timestamp_bounds(self, fetch_json: object, _: object) -> None:
        ...
        self.assertEqual(body["filter"], '(severity>=ERROR) AND timestamp >= "2026-07-31T12:00:00Z" AND timestamp <= "2026-07-31T12:30:00Z"')
        self.assertEqual(body["pageSize"], 10)
        self.assertNotIn("pageToken", body)
        self.assertEqual(observer.log_names_url("matrixed-mind-dev", 10), "https://logging.googleapis.com/v2/projects/matrixed-mind-dev/logs?pageSize=10")
    ...
    @patch.dict(os.environ, {"MATRIXEDMIND_GCP_OPERATOR_EMAIL": "operator@example.test"}, clear=True)
    @patch.object(observer, "command_token", side_effect=["source-token", "impersonated-token"])
    @patch.object(observer, "fetch_json", side_effect=[
        {"email": "operator@example.test"},
        {"name": "locations/global/metricsScopes/123", "monitoredProjects": [
            {"name": "locations/global/metricsScopes/123/projects/123"},
        ]},
        {"status": "success", "data": {"result": [
            {"metric": {"__name__": "run.googleapis.com/request_count", "service_name": "sensitive"}, "values": [[1, "2"]]},
            {"metric": {"__name__": "run.googleapis.com/request_count"}, "values": [[1, "3"]]},
        ]}},
    ])
    def test_query_range_injects_bounds_and_caps_sanitized_result(self, fetch_json: object, _: object) -> None:
        ...
        self.assertEqual(parsed.path, "/v1/projects/matrixed-mind-dev/location/global/prometheus/api/v1/query_range")
        self.assertEqual(query, {"query": ["up"], "start": [VALID["start_time"]],
                                 "end": [VALID["end_time"]], "step": ["60s"], "timeout": ["20s"]})
        self.assertEqual(result["result"], {"series_count": 1, "truncated": True,
    ...
    @patch.object(observer, "fetch_json", return_value={
    ...
    def test_monitoring_queries_reject_cross_project_metrics_scope(self, _: object, __: object) -> None:
    ...
    @patch.object(observer.urllib.request.OpenerDirector, "open", side_effect=urllib.error.HTTPError(
    ...
    def test_http_failure_reports_only_status_and_host(self, _: object) -> None:
        with self.assertRaisesRegex(observer.ObserverError, r"run\.googleapis\.com failed with HTTP 403"):
